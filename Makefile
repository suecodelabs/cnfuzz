...
kind: build
	...
	helm install --wait --timeout 600 $(if $(GIT_COMMIT),cnfuzz-$(subst /,-,$(GIT_COMMIT))) charts/cnfuzz $(if $(GIT_COMMIT),--set image.tag=$(subst /,-,$(GIT_COMMIT)))

kind-clean:
	helm delete $(if $(GIT_COMMIT),$(subst /,-,$(GIT_COMMIT)))
