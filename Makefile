...
DEFAULT_HELM_DEV_ARGS := --set minio.persistence.size=1Gi,minio.resources.requests.memory=1Gi,minio.replicas=1,minio.mode=standalone --set redis.architecture=standalone,redis.replica.replicaCount=1 --set scheduler.restlerConfig.memoryLimit=100,scheduler.restlerConfig.cpuLimit=100,scheduler.restlerConfig.timeBudget=0.001
