...
type EventHandler struct {
	...
	handleFunc      func(clientSet kubernetes.Interface, repositories *repository.Repositories, pod *apiv1.Pod, config config.SchedulerConfig)
...
func (handler EventHandler) OnAdd(obj any) {
	...
	case *apiv1.Pod:
		handler.handleFunc(handler.clientSet, handler.repositories, object, handler.schedulerConfig)
	default:
		return
	}
...
func (handler EventHandler) OnUpdate(oldObj any, newObj any) {
	...
	case *apiv1.Pod:
		...
		handler.handleFunc(handler.clientSet, handler.repositories, newObject, handler.schedulerConfig)
...
func HandlePodEvent(clientSet kubernetes.Interface, repositories *repository.Repositories, pod *apiv1.Pod, config config.SchedulerConfig) {
	logger := log.L()
	...
	fJob := job.CreateFuzzJob()
	_, jErr := fJob.LaunchFuzzJob(clientSet)
	if jErr != nil {
		// TODO get this error to the user
		logger.Errorf("error while starting new fuzzing fJob: %+v", jErr)
	} else {
		logger.Infof("launched fuzzing fJob %s for pod %s", fJob.JobName, pod.Name)
	}
}
