...
type controller struct {
	...
	handleFunc func(l logr.Logger, clientSet kubernetes.Interface, storage *persistence.Storage, config *config.CnFuzzConfig, overwrites config.Overwrites, pod *apiv1.Pod)
}

func NewController(l logr.Logger, client kubernetes.Interface, storage *persistence.Storage, config *config.CnFuzzConfig, overwrites config.Overwrites) *controller {
	return &controller{
		log:        l,
		client:     client,
		storage:    storage,
...
func StartController(l logr.Logger, storage *persistence.Storage, config *config.CnFuzzConfig, overwrites config.Overwrites, client kubernetes.Interface) (err error) {
...
func handlePodEvent(l logr.Logger, client kubernetes.Interface, storage *persistence.Storage, config *config.CnFuzzConfig, overwrites config.Overwrites, pod *apiv1.Pod) {
...
func containsUnfuzzedImages(l logr.Logger, pod *apiv1.Pod, cache persistence.Cache[model.ContainerImage]) (allImages []model.ContainerImage, containsUnfuzzedImages bool) {
