...
func TestFindContainerImageByHashFound(t *testing.T) {
	...
	response, found, err := repo.GetByKey(context.TODO(), hashKey)
	assert.NoError(t, err)
	assert.True(t, found)
...
func TestFindContainerImageByNameNil(t *testing.T) {
	...
	repo := createMocRepo(images)
	response, found, err := repo.GetByKey(context.TODO(), "some-unknown-image-hash")
	assert.NoError(t, err)
...
func createMocRepo(containerImages []*model.ContainerImage) *containerImageMem {
	return &containerImageMem{
		fuzzedImages: containerImages,
	}
}

func createFilledMocRepo() *containerImageMem {
	...
	return &containerImageMem{fuzzedImages: images}
}
